...
def simple3D():
    ...
    gunCool = 0
    missles = []
    roofs = [(-100,100),(200,100),(200,400),(400,300),(600,250),(-400,500)]
    ...
    while running:
        ...
        if gunCool>0:
            gunCool-=1
        if keys[32] and gunCool==0:
            gunCool = 10
            ang = atan2(my-300,mx-400)
            vx = cos(ang)*2
            vy = sin(ang)*2
            missles.append([400,300,vx,vy])
            print(missles)
        ...
        for m in missles:
            ...
            m[0]+=m[2]
            m[1]+=m[3]
            if max(m) > 1000 or min(m) < -100:
                kills.append(m)
        for m in kills:
            missles.remove(m)

        for m in missles:
            draw.circle(screen,(255,0,0),(int(m[0]),int(m[1])), 4)
...
def menu():
    ...
    vals = ["simple3D","instructions","credits"]
    font.init()
    myfont=font.SysFont("Britannic Bold", 30)
    nlabel1=myfont.render("Start", 1, (0, 0, 0,))
    nlabel2=myfont.render("Instructions", 1, (0, 0, 0,))
    nlabel3=myfont.render("Credits", 1, (0, 0, 0,))
    while running:
        for evnt in event.get():          
            if evnt.type == QUIT:
                return "exit"

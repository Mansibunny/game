...
class Gem:
    def __init__(self,pic,x,platY,char,scroll):
        self.startX=x
        self.rect=Rect(x,platY-pic.get_height(),pic.get_width(),pic.get_height())
        self.got=False #False if not collected yet
        self.pic=pic
...
class Map:
    def __init__(self,pic,scroll,me,enemies,kits,gems):
        self.pic=pic
        self.scroll=scroll
        self.x=0
        self.me=me
    ...
    def objectMove(self):
        ...
        if len(self.gems)>0:
            for g in self.gems:
                if self.scroll==True:
                    g[0].move(self.me) #moves kits on screen and in rects
    ...
    def objectCollide(self):
        if len(self.kits)>0:
            for k in self.kits:
                if self.me.rect.colliderect(k[0].rect): #checks if rects collide
                    k[0].gain(self.me)
                    
        if len(self.gems)>0:
            for g in self.gems:
                if self.me.rect.colliderect(g[0].rect): #checks if rects collide
                    g[0].gain(self.me)
...
def story(me):
    me=Player(me)
    dude=[Enemy(enePic,randint(400,1000),500,True)]
    t=Torch()
    kits=[[medKit(medPic,i,500,me,True)] for i in range(400,601,100)] #all medkits
    gems=[[Gem(gem1Pic,900,500,me,True)]] #all gems
    back=Surface((pic.get_width(),pic.get_height()))
    for i in range(2):
        back.blit(pic,(i*pic.get_width(),0))
    MAP=Map(back,True,me,dude,kits,gems)
